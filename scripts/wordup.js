...
function addNewWordSubmission(word) {
    ...
    for (var x = 0; x < model.wordSubmissions.length; x++){
      if (model.wordSubmissions[x].word == word) {
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var x = 0; x < model.wordSubmissions.length; x++){
              if (model.wordSubmissions[x].word == word) {
                model.wordSubmissions[x].isRealWord = theAnswer;
                break;
              }
            }
...
function currentScore() {
    ...
    var wordScoreSum = wordScores.reduce(add, 0);
    return wordScoreSum;
}
